...
int create_child_process(struct process_data_t *p_process_ctl, int process_num) {
    ...
    for (int i = 0; i < process_num; i++) {
        ...
        if (0 == pid) {
            ...
            printf("child %d returns unexpected and has been set to sleep to prevent the process pool from crashing.\n", getpid());
            sleep(10000);
        }
